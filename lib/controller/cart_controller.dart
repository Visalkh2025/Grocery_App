...
class CartController extends GetxController {
  final CartService cartService = CartService();
  Rx<Cart> cart = Cart.empty().obs;
  final discount = 0.obs; // int
  final promoCodeController = TextEditingController().obs;
  final total = 0.obs; // int
  final appliedPromo = Rx<Map<String, dynamic>?>(null);
  ...
  final subTotal = 0.obs; // int

  Future<void> getSubTotal() async {
    int total = 0;
    for (var item in cart.value.items) {
      final price = (item.product?.finalPrice ?? 0).toInt();
      total += item.quantity * price;
    }
    subTotal.value = total;
    _recalculateDiscount();
  ...
  void _recalculateDiscount() {
    ...
    if (promo['discountType'] == "percentage") {
      final discountValue = (promo["discountValue"] ?? 0).toInt();
      discount.value = ((discountValue / 100) * subTotal.value).toInt();
    } else if (promo['discountType'] == "fixed") {
      discount.value = (promo["discountValue"] ?? 0).toInt();
    }
  }

  List<Map<String, dynamic>> getSummaryItems() {
    int shipping = 0;
    int estimatedTaxes = 0;
    int otherFee = 0;
    int total = shipping + estimatedTaxes + otherFee + subTotal.value - discount.value;

    return [
      {"title": "Subtotal", "amount": subTotal.value},
      // {"title": "Address", "amount": shipping, "editable": true},
      {"title": "Estimated Taxes", "amount": estimatedTaxes},
      {"title": "Other Fees", "amount": otherFee},
      {"title": "Discount", "amount": discount.value},
  ...
  Future<void> checkoutCart() async {
    try {
      isLoadingFetchCard(true);
      final totalAmount = getSummaryItems().last['amount'] as int;
      ...
      final items = cart.value.items
          .map(
            (e) => {
              "productId": e.product!.id,
              "quantity": e.quantity,
              "price": e.product!.finalPrice.toInt(),
              "subTotal": (e.product!.finalPrice * e.quantity).toInt(),
            },
          )
          .toList();
